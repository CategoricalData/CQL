...
public class GetInjectivePathState<F extends ModelFrame<F, GM, M, N, E>, GM extends EasikGraphModel, M extends Model<F, GM, M, N, E>, N extends ModelVertex<F, GM, M, N, E>, E extends ModelEdge<F, GM, M, N, E>> extends GetPathState<F, GM, M, N, E> {
	...
	/**
	 * Constructs a new GetInjectivePathState that can begin on any injective
	 * edge.
	 *
	 * @param next
	 ...
	 *            false otherwise.
	 * @param edgeOnly
	 *            Boolean determining whether the user should be selecting just
	 *            a single, injective edge (true), or any path beginning with an
	 *            injective edge (false).
	 * @param inSketch
	 *            The sketch
	 */
	...
	/**
	 * Constructs a new GetInjectivePathState that can begin on any injective
	 * edge out of the desired source node, and must end at the specified target
	 * node.
	 *
	 * @param next
	 *            Boolean determining whether the user should be allowed to
	 ...
	 *            injective edge (false).
	 * @param sourceNode
	 *            EntityNode specifying the node at which the path must start
	 *            (null if it can start on any injective edge).
	 * @param targetNode
	 *            EntityNode specifying the node at which the path must end
	 *            before the user can click next or finish.
	 * @param inSketch
	 *            The sketch
	 */
	public GetInjectivePathState(boolean next, boolean finish, boolean edgeOnly, M inModel, N sourceNode, N targetNode) {
	...
	public Object[] getSelectables() {
		...
		/*
		 * Since we are allowing nodes and edges to be created as we go
		 * everything will be selectable
		 * 
		 */
