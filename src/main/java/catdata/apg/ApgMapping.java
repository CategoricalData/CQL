...
public class ApgMapping<L1, L2> implements Semantics {
  ...
  public final Map<L1, Triple<String, ApgTy<L2>, ApgTerm<L2, Void>>> mapping;
  ...
  public synchronized <E> ApgInstance<L1, Pair<L1, ApgTerm<L2, E>>> delta(ApgInstance<L2, E> G) {
    ...
    for (Entry<L1, Triple<String, ApgTy<L2>, ApgTerm<L2, Void>>> x : mapping.entrySet()) {
      Triple<String, ApgTy<L2>, ApgTerm<L2, Void>> t = x.getValue();
      L1 l1 = x.getKey();
...
//    System.out.println("es " + Util.sep(es, " -> ", " \n "));
  ...
  private static <L2, E> ApgTerm<L2, E> deref(ApgInstance<L2, E> g, ApgTerm<L2, E> t) {
    if (t.proj != null && t.a.fields != null) {
      ...
      Pair<String, ApgTerm<L2, E>> x = t.cases.get(t.a.inj);
  ...
  public static <L> ApgMapping<L, L> id(ApgSchema<L> s) {
    Map<L, Triple<String, ApgTy<L>, ApgTerm<L, Void>>> m = new THashMap<>();
    String v = ("v");
  ...
  public ApgMapping(ApgSchema<L1> src, ApgSchema<L2> dst,
      Map<L1, Triple<String, ApgTy<L2>, ApgTerm<L2, Void>>> mapping) {
    this.src = src;
    this.dst = dst;
  ...
  public void validate() {
    ...
    for (L1 l : src.schema.keySet()) {
      ...
      Triple<String, ApgTy<L2>, ApgTerm<L2, Void>> z = mapping.get(l);
      Map<String, ApgTy<L2>> ctx = new THashMap<>();
  ...
  private ApgTy<L2> type(ApgTerm<L2, Void> term, Map<String, ApgTy<L2>> ctx) {
    if (term.e != null) {
  ...
  public String toString() {
    String s = "labels\n\t"
        + Util.sep(mapping, " -> ", "\n\t", (Triple<String, ApgTy<L2>, ApgTerm<L2, Void>> w) -> "lambda " + w.first
            + " : " + w.second + ". " + w.third);
