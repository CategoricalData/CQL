...
public class AqlProver<Ty, En, Sym, Fk, Att, Gen, Sk> implements DP<Ty, En, Sym, Fk, Att, Gen, Sk> {
	...
	public AqlProver(AqlOptions ops,
		...
		Function<Term<Ty, En, Sym, Fk, Att, Gen, Sk>,Term<Ty, En, Sym, Fk, Att, Gen, Sk>>
		fn;
	...
	private static <Sk, En, Fk, Ty, Att, Sym, Gen> ProverName auto(AqlOptions ops, 
		...
		} else if (!(Boolean) ops.getOrDefault(AqlOption.program_allow_nontermination_unsafe) 
				&& reorientable(col) 
				&& ProgramProver.isProgram(VarIt.it(), reorient(col).toKB(), false) || (Boolean) ops.getOrDefault(AqlOption.program_allow_nontermination_unsafe) 
	...
	public static <Ty, En, Sym, Fk, Att> DP createInstance(AqlOptions options,
			Collage col, Schema<Ty, En, Sym, Fk, Att> schema) {
