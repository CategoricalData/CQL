...
public class EdsExpRaw extends EdsExp implements Raw {
	...
	public EdsExpRaw(SchExp schema, List<EdsExp> imports, List<Pair<Integer, EdExpRaw>> eds, List<Pair<String, String>> options) {
		this.schema = schema;
	...
	public static class EdExpRaw extends Exp<Void> implements Raw {
		...
		private Map<String, List<InteriorLabel<Object>>> raw = new THashMap<>();
			...
			public ED eval(Schema<Ty, En, Sym, Fk, Att> sch, AqlOptions ops) {
				Pair<Map<Var, Chc<Ty,En>>, Set<Pair<Term<Ty, En, Sym, Fk, Att, Void, Void>, Term<Ty, En, Sym, Fk, Att, Void, Void>>>> 
				x = eval1(sch, As, Awh);
			...
			private static Pair<Map<Var, Chc<Ty,En>>, Set<Pair<Term<Ty, En, Sym, Fk, Att, Void, Void>, Term<Ty, En, Sym, Fk, Att, Void, Void>>>> eval1(Schema<Ty, En, Sym, Fk, Att> sch, List<Pair<String, String>> As, Set<Pair<RawTerm, RawTerm>> Awh) {
				...
				for (Pair<RawTerm, RawTerm> eq : Awh) {
						Triple<Map<Var, Chc<Ty, En>>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>>
						eq0 =
						RawTerm.infer1x(As1, eq.first, eq.second, null, col.convert(), "", sch.typeSide.js).first3();
