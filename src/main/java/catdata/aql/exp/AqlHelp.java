    ...
    try {
      ...
      for (Example ex : Util.alphabetical(Examples.getExamples(Language.CQL))) {
        ...
        for (Exp<?> e : prog.exps.values()) {
          if (!set.contains(e.getSyntax())) {
            set.add(e.getSyntax());
            other.append(
                "\t\t\t<a href=\"" + e.kind() + e.getKeyword() + ".html" + "\" >" + e.getKeyword() + "</a><br />\n");
          }
          ...
          try {
            ...
            for (String k : dr.env.defs.insts.keySet()) {
              ...
              insts.append(AqlInACan.toHtml(dr.env,
                  (Instance<String, String, catdata.aql.exp.Sym, catdata.aql.exp.Fk, catdata.aql.exp.Att, String, String, Object, Object>) i));
            }
      ...
      for (Kind k : Util.alphabetical(Arrays.asList(Kind.class.getEnumConstants()))) {
        ...
        for (Pair<AqlTyping, Exp<?>> pair : ee) {
          ...
          Util.writeFile("<html><head>" + css + "</head><h1>" + e.kind() + " " + e.getKeyword() + "</h1>\n<pre>\n" + str
              + "\n\t</pre>\n" + desc + "<br/>" + str2 + "<br/>" + str3 + "</html>", dstFile);
          syntax.append("<a href=\"" + e.kind() + e.getKeyword() + ".html" + "\" target=\"primary\">" + e.getKeyword()
              + "</a><br />\n");
...
//        syntax.append("\t<br");
      ...
      for (AqlOption ex : Util.alphabetical(Arrays.asList(AqlOption.class.getEnumConstants()))) {
        ...
        for (Exp<?> e : opInv.get(ex.toString())) {
          ...
          seen.add(f);
          zzz.append("\t\t\t<a href=\"" + e.kind() + e.getKeyword() + ".html" + "\" target=\"primary\">" + e.getKeyword()
              + "</a><br />\n");
        }
        ...
        for (Example example : Util.alphabetical(Examples.getExamples(Language.CQL))) {
          ...
          if (example.getText().contains(ex.toString())) {
            yyy.append("\n<a href=\"" + example.getName().trim() + ".html\" target=\"primary\">" + example.getName()
                + "</a><br />");
          }
        }

        String sss = "<html><head>" + css + "</head><body><h1>" + "option " + ex.name() + "</h1>\n"
            + AqlHelp.getOptionText(ex) + "<br/><br/>Keyword:</br><br/>" + zzz + "<br/>Appears in:</br><br/>" + yyy
            + "</body></html>";
