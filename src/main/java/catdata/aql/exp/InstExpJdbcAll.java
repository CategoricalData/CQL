...
//					 TINYINT SMALLINT  INTEGER  BIGINT VARCHAR
				...
				for (Map<SqlColumn, Optional<Object>> tuple : tuples) {
					...
					for (SqlColumn c : table.columns) {
						...
						Term<Ty, Void, Sym, Void, Void, Void, Null<?>> xxx
						 = InstExpJdbc.objectToSk(sch, val.orElse(null), i, a, tys0, extraRepr, false, nullOnErr);
		...
		ImportAlgebra<Ty, En, Sym, Fk, Att, Gen, Null<?>> alg = new ImportAlgebra<>(sch, ens0, tys0, fks0, atts0, (x,y)->y, (x,y)->y, dontCheckClosure, Collections.emptySet());
		...
		for (SqlTable table : info.tables) {
			...
			for (SqlColumn c : table.columns) {
				if (col0.atts.containsKey(Att.Att(x, c.name))) {
					throw new RuntimeException("Name collision: table " + c.table.name + " col " + c.name + " against table " + col0.atts.get(Att.Att(En.En(table.name), c.name)).first + "\n\n.Possible solution: set option jdbc_import_col_seperator so as to avoid name collisions.");
				}
