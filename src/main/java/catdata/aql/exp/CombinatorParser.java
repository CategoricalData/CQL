...
public class CombinatorParser implements IAqlParser {
	...
	private static void schExp() {
		...
		ret = Parsers.or(inst, empty, schExpRaw(), var, all, colim, parens(sch_ref), pivot, cod, dom, cod2, dom2);
	...
	private static void pragmaExp() {
		...
		Parser<PragmaExp> var = ident.map(PragmaExpVar::new),
				...
				check2 = Parsers.tuple(token("check_query"), query_ref.lazy(), edsExp(), edsExp()).map(x -> new PragmaExpCheck2(x.b, x.c, x.d)),

				cons = Parsers.tuple(token("assert_consistent"), inst_ref.lazy())
						.map(x -> new PragmaExpConsistent(x.b)),

				ret = Parsers.or(jdbcQuery, check, check2, csvInst, cons, csvTrans, var, sql, js, proc, jdbcInst, jdbcTrans,
						match, parens(pragma_ref));
	...
	private static void instExp() {
		...
		Parser<InstExp<?, ?, ?, ?>> var = ident.map(InstExpVar::new),
				...
				.map(x -> new InstExpFrozen(x.b, x.c)),
		        ...
		        Parser ret = Parsers.or(queryQuotientExpRaw(), sigma_chase, l2, pi, frozen,   instExpRand(),
				instExpCoEq(), instExpJdbcAll(), chase, instExpJdbc(), empty, instExpRaw(), var, sigma, delta, distinct,
				eval, colimInstExp(), dom, cd, anon, except, pivot, cod, instExpCsv(), coeval, parens(inst_ref) );
	...
	private static void transExp() {
		Parser<TransExp< ?, ?, ?, ?, ?, ?, ?, ?>> var = ident.map(TransExpVar::new),
				...
				frozen = Parsers.tuple(
						Parsers.tuple(token("frozen"), query_ref.lazy().followedBy(token("lambda"))).map(x->x.b), 
						ident.followedBy(token(":")), 
						ident.followedBy(token(".")), 
						term().followedBy(token(":")), 
						ident)
				.map(x->new TransExpFrozen( x.a, x.b, x.c, x.d, x.e)),
				
				pi = Parsers
						.tuple(token("pi"), map_ref.lazy(), trans_ref.lazy(), options.between(token("{"), token("}"))
				...
				eval = Parsers.tuple(token("eval"), query_ref.lazy(), trans_ref.lazy(), options.between(token("{"), token("}")).optional())
						.map(x -> new TransExpEval(x.b, x.c, x.d==null?new LinkedList() : Util.newIfNull(x.d))),
				except = Parsers.tuple(token("except_return"), inst_ref.lazy(), inst_ref.lazy()).map(x -> new TransExpDiffReturn(x.b, x.c)),		
				except2 = Parsers.tuple(token("except"), trans_ref.lazy(), inst_ref.lazy()).map(x -> new TransExpDiff(x.c, x.b)),		
		...
		Parser ret = Parsers.or(id, id1, transExpRaw(), var, sigma, delta, unit, counit, distinct, eval, coeval,
				transExpCsv(), pi, distinct_return, frozen, except2, unitq, except, counitq, transExpJdbc(), comp, parens(trans_ref));
...
//			.map(x -> x == null ? new LinkedList() : x.b);
	...
	private static final <X> Parser<List<X>> imports(Parser<X> p) {
		return Parsers.tuple(token("imports"), p.many()).optional().map(x -> x == null ? new LinkedList() : x.b);
	}
...
//				.map(x -> new catdata.Pair(x.a, x.b));
		Parser<List<TyExp>> ly = Parsers.tuple(token("imports"), ty_ref.lazy().many()).map(x -> x.b)
				.optional();
	...
	private static Parser<EdsExp> edsExp() {
		Parser<EdsExp> 
                ...
                empty = Parsers.tuple(token("empty"), token(":"), sch_ref.lazy()).map(x->new EdsExpRaw(x.c, new LinkedList(), new LinkedList(), new Object())),
	...
	private static Parser<catdata.Pair<LocStr, PreBlock>> preblock(boolean isSimple) {
		...
		Parser<catdata.Pair<LocStr, PreBlock>> ret	=	ret2.map(x -> new catdata.Pair(x.a, new PreBlock(Util.newIfNull(x.b.a), Util.newIfNull(x.b.b),
				x.b.c==null?new LinkedList():Util.newIfNull(x.b.c.b), Util.newIfNull(x.b.d), Util.newIfNull(x.c),x.b.c==null?false:x.b.c.a)));
	...
	private static void queryExp() {
		...
		deltaQueryCoEval = Parsers
						...
						.map(x -> new QueryExpDeltaCoEval((MapExp) x.b, Util.newIfNull(x.c))),
		...
		fromConstraints = Parsers.tuple(token("fromConstraints"), ident, eds_ref.lazy()).map(x->new QueryExpFromEds(x.c,Integer.parseInt(x.b))),
		
		
		ret = Parsers.or(id, id2, fromCoSpan, fromConstraints, queryExpRaw(), queryExpRawSimple(), var, deltaQueryEval, deltaQueryCoEval, comp,
						parens(query_ref));
	...
	private static Parser<InstExpColim<String, String, String, String, String, String>> colimInstExp() {
		...
		@SuppressWarnings({ "rawtypes", "unchecked" })
		Parser<InstExpColim<String, String, String, String, String, String>> ret = Parsers
	...
	private static Parser<MapExpRaw> mapExpRaw() {
		...
		Parser<List<String>> p2 = token("identity").map(x->new LinkedList());
	...
	private static Parser<Program<Exp<?>>> program(String s) { 
		...
		return Parsers.tuple(options, p.many()).map(x -> new Program((x.b), s, x.a, q -> ((EdsExp) q).kind().toString()));
	}
