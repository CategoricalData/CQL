...
public class PragmaExpCheck2 extends PragmaExp {
	...
	public PragmaExpCheck2(QueryExp q, EdsExp c,
			EdsExp d) {
		Q = q;
		C = c;
		D = d;
	...
	public Pragma eval0(AqlEnv env, boolean isC) {
		return new Pragma() {
			...
			public void execute() {
				...
				KBTheory<Chc<Ty, En>, Head<Ty, En, Sym, Fk, Att, Object, Object>, Var> 
				kb = C0.schema.collage().toKB(); 
				
				String s1 = C0.tptp("axiom", i, true, kb);
				String s2 = C1.tptp("conjecture", i, false, kb);
				boolean b = EProver.check(exePath, timeout, s1 + "\n" + s2);
				if (!b) {
