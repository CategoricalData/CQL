...
public final class InstExpSigma<Gen, Sk, X, Y> extends InstExp<Gen, Sk, Integer, Chc<Sk, Pair<Integer, Att>>> {
	...
	@Override
	public synchronized SigmaInstance<Ty, En, Sym, Fk, Att, Gen, Sk, En, Fk, Att, X, Y> eval0(AqlEnv env, boolean isC) {
		Mapping<catdata.aql.exp.Ty, En, catdata.aql.exp.Sym, Fk, Att, En, Fk, Att> F = this.F.eval(env, isC);
		Instance<Ty, En, Sym, Fk, Att, Gen, Sk, X, Y> I = this.I.eval(env, isC);
		if (isC) {
