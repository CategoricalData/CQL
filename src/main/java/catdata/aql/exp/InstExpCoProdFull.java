...
public final class InstExpCoProdFull<Gen, Sk, X, Y>
  ...
  @Override
  public synchronized Instance<String, String, Sym, Fk, Att, Pair<String, Gen>, Pair<String, Sk>, Integer, Chc<Pair<String, Sk>, Pair<Integer, Att>>> eval0(
      AqlEnv env, boolean isC) {
    Schema<String, String, Sym, Fk, Att> sch0 = sch.eval(env, isC);

    Collage<String, String, Sym, Fk, Att, Pair<String, Gen>, Pair<String, Sk>> col = new CCollage<>();
    AqlOptions strat = new AqlOptions(options, env.defaults);
    Set<Pair<Term<String, String, Sym, Fk, Att, Pair<String, Gen>, Pair<String, Sk>>, Term<String, String, Sym, Fk, Att, Pair<String, Gen>, Pair<String, Sk>>>> eqs0 = (new THashSet<>());

    Map<String, Instance<String, String, Sym, Fk, Att, Gen, Sk, X, Y>> m = (new THashMap<>());
    boolean onlyFree = true;
    for (String x : Is) {
      Instance<String, String, Sym, Fk, Att, Gen, Sk, X, Y> I = (Instance<String, String, Sym, Fk, Att, Gen, Sk, X, Y>) new InstExpVar(
          x).eval(env, isC);
    ...
    if (onlyFree) {
      CoprodInstance<String, String, Sym, Fk, Att, Gen, Sk, X, Y> k = new CoprodInstance<>(m, sch0,
          (boolean) strat.getOrDefault(AqlOption.allow_java_eqs_unsafe),
          (boolean) strat.getOrDefault(AqlOption.require_consistency));

      Object o = k;
      return (Instance<String, String, Sym, Fk, Att, Pair<String, Gen>, Pair<String, Sk>, Integer, Chc<Pair<String, Sk>, Pair<Integer, Att>>>) o;
    }

    for (String x : Is) {
      Instance<String, String, Sym, Fk, Att, Gen, Sk, X, Y> I = m.get(x);
      I.gens().keySet((g) -> {
    ...
    InitialAlgebra<String, String, Sym, Fk, Att, Pair<String, Gen>, Pair<String, Sk>> initial0 = new InitialAlgebra<>(strat,
        sch0, col, (y) -> y, (x, y) -> y);
