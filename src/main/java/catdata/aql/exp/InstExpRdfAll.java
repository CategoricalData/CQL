...
public class InstExpRdfAll extends InstExp<String, Null<?>, String, Null<?>> {
  ...
  public synchronized Instance<String, String, Sym, Fk, Att, String, Null<?>, String, Null<?>> eval0(AqlEnv env, boolean isC) {
    ...
    Map<String, Collection<String>> ens0 = new THashMap<>(2);
  

    Map<String, Map<String, Map<Fk, String>>> fks0x = new THashMap<>(2, 2);
    Map<String, Map<String, Map<Att, Term<String, Void, Sym, Void, Void, Void, Null<?>>>>> atts0x = new THashMap<>(2, 2);
    ...
    Map<String, Map<Att, Term<String, Void, Sym, Void, Void, Void, Null<?>>>> atts0 = new THashMap<>((int) model.size(), 2);
    atts0x.put(("R"), atts0);
    Map<String, Collection<Null<?>>> tys0 = new THashMap<>(2, 2);
    tys0.put(("Dom"), new THashSet<>(1024 * 16));
    ...
    while (iter.hasNext()) {
      ...
      Map<Att, Term<String, Void, Sym, Void, Void, Void, Null<?>>> map = new THashMap<>(3, 2);
      ...
      if (subject.isAnon()) {
        ...
        map.put(Att.Att(("R"), "subject"), Term.Obj(subject.toString(), ("Dom")));
      }
      if (object.isAnon()) {
        ...
        map.put(Att.Att(("R"), "object"), Term.Obj(object.toString(), ("Dom")));
      }
    ...
    Schema<String, String, Sym, Fk, Att> sch = new SchExpRaw(new TyExpRdf(), Collections.emptyList(), ensX, fksX,
        Collections.emptyList(), l, Collections.emptyList(), Collections.emptyList(), Unit.unit, null).eval(env, isC);

    // System.out.println(atts0);
    ImportAlgebra<String, String, Sym, Fk, Att, String, Null<?>> alg = new ImportAlgebra<>(sch, x->ens0.get(x), tys0, (en, x) -> fks0x.get(en).get(x), (en, x) -> atts0x.get(en).get(x),
