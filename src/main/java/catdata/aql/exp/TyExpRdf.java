...
public final class TyExpRdf extends TyExp {
	...
	public synchronized TypeSide<Ty, Sym> eval0(AqlEnv env, boolean isC) {
		...
		List<Pair<LocStr, String>> jtys = Collections.singletonList(new Pair<>(new LocStr(0, "Dom"), "java.lang.String"));
		List<Pair<LocStr, String>> jps = Collections.singletonList(new Pair<>(new LocStr(0, "Dom"), "x => x"));
		return new TyExpRaw(Collections.emptyList(), Collections.emptyList(), Collections.emptyList(), Collections.emptyList(), jtys, jps, Collections.emptyList(), Collections.emptyList()).eval(env, isC);
	}
