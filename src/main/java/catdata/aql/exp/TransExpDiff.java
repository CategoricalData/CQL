...
extends TransExp<X1, Y, X2, Y, X1, Y, X2, Y> {
	...
	public TransExpDiff(
			InstExp<Gen, Sk, X, Y> i, TransExp<Gen1, Sk1, Gen2, Sk2, X1, Y, X2, Y> h) {
		this.h = h;
		I = i;
	}
	...
	@Override
	public Transform<Ty, En, Sym, Fk, Att, X1, Y, X2, Y, X1, Y, X2, Y> eval0(AqlEnv env, boolean isC) {
		Instance<Ty, En, Sym, Fk, Att, Gen, Sk, X, Y> i = I.eval(env, isC);
		Transform<Ty, En, Sym, Fk, Att, Gen1, Sk1, Gen2, Sk2, X1, Y, X2, Y> H = h.eval(env, isC);
		if (isC) {
