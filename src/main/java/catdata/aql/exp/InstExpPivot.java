...
public final class InstExpPivot<Gen, Sk, X, Y> extends InstExp<X, Y, X, Y> {
  ...
  @Override
  public synchronized Instance<String, String, Sym, catdata.aql.exp.Fk, catdata.aql.exp.Att, X, Y, X, Y> eval0(
      AqlEnv env, boolean isC) {
    if (isC) {
      throw new IgnoreException();
    }
    AqlOptions strat = new AqlOptions(ops, env.defaults);
    Instance<String, String, Sym, catdata.aql.exp.Fk, catdata.aql.exp.Att, X, Y, X, Y> l = new AqlPivot<>(
