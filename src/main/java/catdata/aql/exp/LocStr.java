...
public class LocStr {
	...
	//TODO CQL really, no one should use locstr equality, but right now mapexpraw does 
	...
	public static Map<Pair<String, Object>, Pair<Integer, Function<Pair<String, Object>, String>>>
	...
	public static <X> Set<Pair<String,Pair<String, X>>> set2x(Collection<Pair<LocStr, Pair<String, String>>> atts, Function<String, X> f) {
		return atts.stream().map(x -> new Pair<>(x.first.str, new Pair<>(x.second.first, f.apply(x.second.second)))).collect(Collectors.toUnmodifiableSet());
	}
	
	public static <En,Y> Set<Pair<String, En>> set2y(Collection<Pair<LocStr, Y>> gens, Function<Y, En> f) {
		return gens.stream().map(x -> new Pair<>(x.first.str, f.apply(x.second))).collect(Collectors.toUnmodifiableSet());
	}
...
*/
	...
	public static <X,Y> List<Pair<Pair<Y, String>, X>> list2x(List<Pair<Pair<Y,LocStr>, X>> as) {
		return as.stream().map(x -> new Pair<>(new Pair<>(x.first.first, x.first.second.str),x.second)).collect(Collectors.toUnmodifiableList());
	}
	...
	public static <X,Y> List<Pair<Y, X>> list2(List<Pair<LocStr, X>> as, Function<String,Y> f) {
		return as.stream().map(x -> new Pair<>(f.apply(x.first.str), x.second)).collect(Collectors.toUnmodifiableList());
	}
/*
