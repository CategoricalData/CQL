...
public final class ColimSchExpModify<N> extends ColimSchExp<N> implements Raw {
	...
	public synchronized ColimitSchema<N> eval0(AqlEnv env, boolean isC) {
		...
		ColimitSchema<N> colim0 = colim.eval(env, isC);
		...
		for (Pair<Pair<String, String>, String> k : fks0) {
			colim0 = colim0.renameFk(Fk.Fk(En.En(k.first.first), k.first.second), Fk.Fk(En.En(k.first.first), k.second), checkJava);
		}
		for (Pair<Pair<String, String>, String> k : atts0) {
			colim0 = colim0.renameAtt(Att.Att(En.En(k.first.first), k.first.second), Att.Att(En.En(k.first.first), k.second), checkJava);
		}
		for (Pair<Pair<String, String>, List<String>> k : fks) {
			...
			RawTerm.infer1x(Map, term, null, Chc.inRight(tr), xxx.convert(), pre, colim0.schemaStr.typeSide.js).second;
		...
		for (Pair<Pair<String, String>, Triple<String, String, RawTerm>> k : atts) {
			...
			Term<Ty, En, Sym, Fk, Att, Gen, Sk> t = 
			RawTerm.infer1x(Map, k.second.third, null, Chc.inLeft(r.second), xxx.convert(), pre, colim0.schemaStr.typeSide.js).second;
			colim0 = colim0.removeAtt(Att.Att(En.En(k.first.first), k.first.second), Var.Var(k.second.first), t.convert(), checkJava);
		}
