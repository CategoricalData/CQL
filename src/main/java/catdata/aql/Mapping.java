...
public final class Mapping<Ty,En1,Sym,Fk1,Att1,En2,Fk2,Att2> implements Semantics {
	...
	public synchronized Schema<Ty,Chc<En1,En2>,Sym,Chc<Chc<Fk1,Fk2>,En1>,Chc<Att1,Att2>> collage() {
		...
		for (Fk1 a : src.fks.keySet()) {
			...
			Term<Ty, Chc<En1, En2>, Sym, Chc<Chc<Fk1,Fk2>,En1>, Chc<Att1, Att2>, Void, Void> lhs
			= Term.Fk(Chc.inRight(w), Term.Fk(Chc.inLeft(Chc.inLeft(a)), Term.Var(xxx)));
			
			Term<Ty, Chc<En1, En2>, Sym, Chc<Chc<Fk1,Fk2>,En1>, Chc<Att1, Att2>, Void, Void> rhs
			= Term.Fks(Chc.inLeft(Chc.inRight(fks.get(a).second)), Term.Fk(Chc.inRight(v), Term.Var(xxx)));
		...
		for (Att1 a : src.atts.keySet()) {
			...
			Term<Ty, Chc<En1, En2>, Sym, Chc<Chc<Fk1,Fk2>,En1>, Chc<Att1, Att2>, Void, Void> rhs 
			= term.subst(Collections.singletonMap(x, Term.Fk(Chc.inRight(v), Term.Var(x))));
	...
	private Map<Chc<Att1, Att2>, Pair<Chc<En1, En2>, Ty>> or(Map<Att1, Pair<En1, Ty>> xs,
			Map<Att2, Pair<En2, Ty>> ys) {
	...
	private <Att1, Att2> Map<Chc<Chc<Att1, Att2>,En1>, Pair<Chc<En1, En2>, Chc<En1,En2>>> or2(Map<Att1, Pair<En1, En1>> xs,
			Map<Att2, Pair<En2, En2>> ys) {
		Map<Chc<Chc<Att1, Att2>,En1>, Pair<Chc<En1, En2>, Chc<En1,En2>>> ret = new THashMap<>(xs.size() + ys.size() + src.ens.size());
		for (Att1 att : xs.keySet()) {
	...
	public Morphism<Ty,En1,Sym,Fk1,Att1,Void,Void,En2,Sym,Fk2,Att2,Void,Void> semantics() {
		...
		semantics = new Morphism<>() {
			...
			@Override
			public Pair<Map<Var, Chc<Ty, En2>>, Term<Ty, En2, Sym, Fk2, Att2, Void, Void>> translate(Map<Var, Chc<Ty, En1>> ctx, Term<Ty, En1, Sym, Fk1, Att1, Void, Void> term) {
	...
	public static <Ty,En,Sym,Fk,Att> Mapping<Ty,En,Sym,Fk,Att,En,Fk,Att> id(Schema<Ty,En,Sym,Fk,Att> s, Optional<Schema<Ty,En,Sym,Fk,Att>> t) {
	...
	public void validate(boolean doNotCheckEquations) {
		...
		if (!doNotCheckEquations) {
			for (Triple<Pair<Var, En1>, Term<Ty, En1, Sym, Fk1, Att1, Void, Void>, Term<Ty, En1, Sym, Fk1, Att1, Void, Void>> eq : src.eqs) {
				...
				boolean ok = dst.dp.eq(Collections.singletonMap(eq.first.first, Chc.inRight(ens.get(eq.first.second))), lhs, rhs);
				if (!ok) {
