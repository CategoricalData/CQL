...
class Inferrer {
	...
	private static <Ty, En, Sym, Fk, Att, Ty0, En0, Sym0, Fk0, Att0> String inferTrans(Fk0 fk, Schema<Ty, En, Sym, Fk, Att> a, Schema<Ty0, En0, Sym0, Fk0, Att0> b) {
		...
		String gens = Util.sep(Util.alphabetical(dom).stream().map(z -> z + " -> a generator [" + pr(cod) + "] ." + " a path of foreign keys [" + pr3(a.fks) + "]").collect(Collectors.toList()), "\n\t\t\t\t");
		return "{" + gens + "}";
