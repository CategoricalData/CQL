...
public class ColimitInstance<N, E, Ty, En, Sym, Fk, Att, Gen, Sk, X, Y> 
 extends Instance<Ty, En, Sym, Fk, Att, Pair<N,Gen>, Pair<N,Sk>, Integer, Chc<Pair<N,Sk>, Pair<Integer, Att>>> {
	...
	public ColimitInstance(Schema<Ty, En, Sym, Fk, Att> schema, DMG<N, E> shape, Map<N, Instance<Ty, En, Sym, Fk, Att, Gen, Sk, X, Y>> nodes, Map<E, Transform<Ty, En, Sym, Fk, Att, Gen, Sk, Gen, Sk, X, Y, X, Y>> edges, AqlOptions options) {
		...
		for (N n : nodes.keySet()) {
			...
			for (Pair<Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>> eq : nodes.get(n).eqs()) {
				col.eqs.add(new Eq<>(null, eq.first.mapGenSk(x -> new Pair<>(n, x), x -> new Pair<>(n, x)), eq.second.mapGenSk(x -> new Pair<>(n, x) ,x -> new Pair<>(n, x))));
		...
		for (E e : shape.edges.keySet()) {
			...
			for (Gen gen : h.src().gens().keySet()) {
				...
				col.eqs.add(new Eq<>(null, Term.Gen(new Pair<>(shape.edges.get(e).first, gen)), rhs.map(Util.voidFn(), Util.voidFn(), Function.identity(), Util.voidFn(), x -> new Pair<>(shape.edges.get(e).second, x), Util.voidFn())));
			}
...
//		BiFunction<En,Pair<N,Gen>,Object> printGen = (y,x) -> nodes.get(x.first).algebra().printX(y,nodes.get(x.first).algebra().nf(Term.Gen(x.second)));
		...
		BiFunction<Ty,Pair<N,Sk>, Object> printSk = (y,x) -> nodes.get(x.first).algebra().sk(x.second).toString(w->nodes.get(x.first).algebra().printY(y,w).toString(), Util.voidFn());
		
		InitialAlgebra<Ty, En, Sym, Fk, Att, Pair<N,Gen>, Pair<N,Sk>> initial 
		= new InitialAlgebra<>(options, schema(), col, printGen, printSk);
				
		J = new LiteralInstance<>(schema(), col.gens, col.sks, eqs, initial.dp(), initial, (Boolean) options.getOrDefault(AqlOption.require_consistency), (Boolean) options.getOrDefault(AqlOption.allow_java_eqs_unsafe)); 
		//validate();
	}
