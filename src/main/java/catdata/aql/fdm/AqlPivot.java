...
public class AqlPivot<Ty, En0, Sym, Fk0, Att0, Gen, Sk, X, Y> {
	...
	public AqlPivot(Instance<Ty, En0, Sym, Fk0, Att0, Gen, Sk, X, Y> i, AqlOptions strat) {
		...
		for (En0 en : I.schema().ens) {
			for (X x0 : I.algebra().en(en)) {
				...
				for (Att0 att : I.schema().attsFrom(en)) {
					...
					Map<Att, Term<Ty, Void, Sym, Void, Void, Void, Y>> 
					ctx0 = attsX.get(x0);
					if (ctx0 == null) {
						ctx0 = new THashMap<>();
					}
		...
		Algebra<Ty, En, Sym, Fk, Att, X, Y, X, Y> initial = 
				new ImportAlgebra<>(intI, ensX, tysX, fksX, attsX, (x,y)->y, (x,y)->y, (Boolean) strat.getOrDefault(AqlOption.allow_java_eqs_unsafe), eqsX);
		...
		J = new LiteralInstance<>
		(intI, col.gens, col.sks, eqs0, dp2, initial,
	...
	private Term<Ty, En0, Sym, Fk0, Att0, Gen, Sk> trans(En en, Term<Ty, En, Sym, Fk, Att, Void, Void> t) {
		if (t.obj() != null) {
			...
			return Term.Att((Att0)Att.Att(t.att().en,t.att().str), x.map(Util::abort, Util::abort, Function.identity(), Util::abort, Function.identity(), Util::abort));
		} 
		return Util.anomaly();
	}
