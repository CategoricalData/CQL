...
public class CoEvalInstance<Ty, En1, Sym, Fk1, Att1, Gen, Sk, En2, Fk2, Att2, X, Y> extends
  ...
  public CoEvalInstance(Query<Ty, En1, Sym, Fk1, Att1, En2, Fk2, Att2> Q,
    ...
    col.sks().putAll(
        Util.map(J.algebra().talg().sks, (k, v) -> new Pair<>(Chc.<Triple<String, X, En2>, Y>inRight(k), v)));
      ...
      for (X j : J.algebra().en(t)) {
        ...
        for (Fk2 fk : J.schema().fksFrom(t)) {
          ...
          fk0.src().gens().entrySet((v0, k) -> {
            Term<Ty, En1, Sym, Fk1, Att1, Triple<String, X, En2>, Chc<Triple<String, X, En2>, Y>> rhs = fk0
                .gens().apply(v0, k).map(Util.voidFn(), Util.voidFn(), Function.identity(),
                    Util.voidFn(), x -> new Triple<>(x, j, t), Util::abort);
        ...
        for (Att2 att : J.schema().attsFrom(t)) {
          ...
          Chc<Term<Ty, En1, Sym, Fk1, Att1, String, String>, Agg<Ty, En1, Sym, Fk1, Att1>> att1 = Q.atts
              .get(att);
          if (!att1.left) {
          ...
          Term<Ty, En1, Sym, Fk1, Att1, Triple<String, X, En2>, Chc<Triple<String, X, En2>, Y>> rhs = att1.l
              .mapGenSk(x -> new Triple<>(x, j, t), x -> Chc.inLeft(new Triple<>(x, j, t)));
          Term<Ty, En1, Sym, Fk1, Att1, Triple<String, X, En2>, Chc<Triple<String, X, En2>, Y>> lhs = J
              .algebra().att(att, j).map(Function.identity(), Function.identity(), Util.voidFn(),
    ...
    Function<Triple<String, X, En2>, Object> printGen = (x) -> "[" + x.first + "="
