...
public class SigmaInstance<Ty, En1, Sym, Fk1, Att1, Gen, Sk, En2, Fk2, Att2, X, Y> 
 extends Instance<Ty, En2, Sym, Fk2, Att2, Gen, Sk, Integer, Chc<Sk, Pair<Integer, Att2>>> {
	...
	public SigmaInstance(Mapping<Ty, En1, Sym, Fk1, Att1, En2, Fk2, Att2> f, Instance<Ty, En1, Sym, Fk1, Att1, Gen, Sk, X, Y> i, AqlOptions strat) {
		...
		BiFunction<Ty,Sk,  Object> printSk = (y,x) -> I.algebra().sk(x).toString(z->I.algebra().printY(y,z).toString(), Util.voidFn());
		InitialAlgebra<Ty, En2, Sym, Fk2, Att2, Gen, Sk> initial 
		= new InitialAlgebra<>(strat, schema(), col, printGen, printSk);
		J = new LiteralInstance<>(schema(), col.gens, col.sks, eqs, initial.dp(), initial, (Boolean) strat.getOrDefault(AqlOption.require_consistency), (Boolean) strat.getOrDefault(AqlOption.allow_java_eqs_unsafe)); 
		validate();
	}
