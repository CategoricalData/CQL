...
public final class Term<Ty, En, Sym, Fk, Att, Gen, Sk> implements KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> {
  ...
  private static <Ty, En, Sym, Fk, Att, Gen, Sk, Ty2, En2, Sym2, Fk2, Att2, Gen2, Sk2> Term<Ty2, En2, Sym2, Fk2, Att2, Gen2, Sk2> map(
      Term<Ty, En, Sym, Fk, Att, Gen, Sk> term, Function<Ty, Ty2> tyf, Function<Sym, Sym2> symf, Function<Fk, Fk2> fkf,
      Function<Att, Att2> attf, Function<Gen, Gen2> genf, Function<Sk, Sk2> skf) {
    return term.visit(Term::Var, (obj, ty) -> Obj(obj, tyf.apply(ty)), (sym, args) -> Sym(symf.apply(sym), args),
        (fk, arg) -> Fk(fkf.apply(fk), arg), (att, arg) -> Att(attf.apply(att), arg), gen -> Gen(genf.apply(gen)),
        sk -> Sk(skf.apply(sk)));
  }
  ...
  public Chc<Ty, En> type(Map<String, Ty> ctxt, Map<String, En> ctxe, Set<Ty> tys, Map<Sym, Pair<List<Ty>, Ty>> syms,
    ...
    if (var != null) {
      ...
      if (t == null) {
        ...
        throw new RuntimeException(
            "In " + this + ", " + sym() + " given " + args.size() + "arguments but requires " + t.first.size());
      }
      for (int i = 0; i < t.first.size(); i++) {
        ...
        if (!Chc.inLeft(t.first.get(i)).equals(u)) {
          throw new RuntimeException("In " + this + ", " + "Argument " + args.get(i) + " has sort " + u.toStringMash()
              + " but requires " + t.first.get(i));
        }
      }
      ...
      if (!Chc.inRight(t.first).equals(u)) {
        ...
        throw new RuntimeException(
            "In " + this + ", " + "argument " + arg + " has sort " + u.toStringMash() + " but requires " + t.first);
      }
      ret = Chc.inLeft(t.second);
    } else if (fk() != null) {
      Chc<Ty, En> u = arg.type(ctxt, ctxe, tys, syms, java_tys_string, ens, atts, fks, gens, sks);
      ...
      if (tye == null) {
        ...
        throw new RuntimeException(
            "In " + this + ", " + "the labelled null " + sk() + " has no associated type.\n\nAvailable: " + xxx);
      }
      ret = Chc.inLeft(tye);
    }
    if (ret == null || (ret.left && ret.l == null) || (!ret.left && ret.r == null)) {
      throw new RuntimeException(
          "In " + this + "," + " typing encountered an ill-formed term.  Should be impossible, report to Ryan.  " + this);
  ...
  public String toStringSql(Schema sch, String tick, boolean convert) {
    if (var != null) {
      ...
      return sym().toString() + "("
          + Util.sep(args.stream().map(x -> x.toStringSql(sch, tick, convert)).collect(Collectors.toList()), ", ") + ")";
  ...
  public String toString(Function<Sk, String> sk_printer, Function<Gen, String> gen_printer) {
    if (var != null) {
      ...
      if (args == null || args.isEmpty()) {
        ...
        return sym().toString() + "("
            + Util.sep(args.stream().map(x -> x.toString(sk_printer, gen_printer)).collect(Collectors.toList()), ", ")
            + ")";
      }
    } else if (att() != null) {
  ...
  public synchronized Term<Ty, En, Sym, Fk, Att, Gen, Sk> subst(Map<String, Term<Ty, En, Sym, Fk, Att, Gen, Sk>> map) {
