...
public class TalgSimplifier<Ty, En, Sym, Fk, Att, Gen, Sk, X, Y> {
	...
	class Step {
		...
		@SuppressWarnings("hiding")
		// public final Map<Head<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X,
		// Att>>>, Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>> s0 =
		// subst;
		...
		public Step(Step s) {
			...
			Iterator<Pair<Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>, Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>>> it = new Util.FilterTransfomIterator<>(
					in.eqsNoDefns().iterator(), (x)->fn(x,errs));
			while (it.hasNext()) {
				out.eqsNoDefns().add(it.next());
			}
			if (!errs.isEmpty()) {
				throw new RuntimeException("Contradictions: \n" + Util.sep(errs, "\n"));
			}
		}
		...
		private synchronized Optional<Pair<Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>, Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>>> fn(
				Pair<Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>, Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>> x, Set<String> errs) {
			Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>> l = simpl(x.first), r = simpl(x.second);
		
			if (l.equals(r)) {
				return Optional.empty();
			}
			var a = sch.typeSide.js.reduce(l);
			var b = sch.typeSide.js.reduce(r);
			if (a.equals(b)) {
				return Optional.empty();
			} 
			if (a.obj() instanceof BigDecimal && b.obj() instanceof BigDecimal) {
				BigDecimal aa = (BigDecimal) a.obj();
				BigDecimal bb = (BigDecimal) b.obj();
				BigDecimal cc = aa.subtract(bb);
				double dd = Math.abs(cc.doubleValue());
				
				if ( dd == 0) { //< .00001  /* || Math.abs(dd / aa.add(bb).doubleValue()) < .01 */ ) { //TODO: floating point
					
				//	System.out.println("Trigger");
					return Optional.empty();
				} else {
				//	System.out.println("Trigger");
				//	System.out.println(a + " and " + b + " is " + dd + " vs " +  Math.abs(cc.doubleValue() / aa.add(bb).doubleValue()));
				}
			}
		...
		private synchronized boolean talg_h1() {
			boolean b = false;
			for (Pair<Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>, Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>> eq : in
					.eqsNoDefns()) {
				b = talg_h(eq) | b;
			}
			return b;
		...
		private synchronized boolean talg_h(
				Pair<Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>, Term<Ty, Void, Sym, Void, Void, Void, Chc<Sk, Pair<X, Att>>>> eq) {
	...
	public TalgSimplifier(Algebra<Ty, En, Sym, Fk, Att, Gen, Sk, X, Y> alg,
			Iterator<Pair<Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>>> it,
			Map<Sk, Ty> colsks, int reduce) {
		this.alg = alg;

		subst = new THashMap<>(alg.estimateNullSize(.1f));
		this.sch = alg.schema();

		TAlg<Ty, Sym, Chc<Sk, Pair<X, Att>>> in = new TAlg<>(new THashMap<>(alg.estimateNullSize(.1f)),
				new LinkedList<>());
	...
	private synchronized void talg_h0(TAlg<Ty, Sym, Chc<Sk, Pair<X, Att>>> in,
			Iterator<Pair<Term<Ty, En, Sym, Fk, Att, Gen, Sk>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>>> it,
			Map<Sk, Ty> colsks) {
		colsks.forEach((a, b) -> {
...
//    in.validate();
		...
		for (Triple<Pair<String, En>, Term<Ty, En, Sym, Fk, Att, Void, Void>, Term<Ty, En, Sym, Fk, Att, Void, Void>> eq : alg
				.schema().eqs) {
			...
			for (X x : alg.en(eq.first.second)) {
				...
				Map<String, Term<Ty, En, Sym, Fk, Att, Void, Void>> map = Collections.singletonMap(eq.first.first, q);
