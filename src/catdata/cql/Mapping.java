...
public final class Mapping<Ty, En1, Sym, Fk1, Att1, En2, Fk2, Att2> implements Semantics {
	...
	public static String coq(String s) {
	...
	public static <Ty,En,Sym,Fk,Att> String coq(Term<Ty,En,Sym,Fk,Att,Void,Void> s, Set<String> sb, Function<Att,En> fk1, Function<Fk,En> fk2, String pre) {
		Function<String,String> fn = (y)->coq(y);
		return s.toCoq(fn, sb, fk1, fk2, pre);
	}
	public static <Ty> String  toCoq(Map<String, Ty> m) {
		String ret = "";
		 for (var x : m.entrySet()) {
			 ret += "(" + (coq(x.getKey()) + " : " + coq(x.getValue().toString())) + ")";
		 }
		 if (!ret.isEmpty())
			 ret = "forall " + ret + ", ";
		 return ret;
	}
	public static <Ty> String  toCoq(String s, Ty m) {
		String ret = "";
			 ret += "(" + (coq(s) + " : " + coq(m.toString())) + ")";
			 if (!ret.isEmpty())
				 ret = "forall " + ret + ", ";
		 return ret;
	}
	...
	public String toCoq() {
		...
		for (var x : src.typeSide.tys) {
			sb.append("Parameter " + coq(x.toString()) + " : Type.\n");
		}
		for (var x : src.typeSide.syms.entrySet()) {
			sb.append("Parameter " + coq(x.getKey().toString()) + " : " + (Util.sep(x.getValue().first.stream().map((y)->coq(y.toString())).collect(Collectors.toList()), " * ")) + (x.getValue().first.isEmpty() ? " " : " -> ") + coq(x.getValue().second.toString()) + ".\n");			
		}
		int i = 0;
		StringBuffer sb2 = new StringBuffer();
		Set<String> sb3 = new HashSet<String>();
		
		Function<Att2, En2> fk1 = (k)->dst.atts.get(k).first;
		Function<Fk2, En2> fk2 = (k)->dst.fks.get(k).first;

		for (var x : src.typeSide.eqs) {
			sb.append("Axiom ax" + (i++) + " : " + toCoq(x.first) + " " + coq(x.second.convert(), sb3, fk1, fk2, "") + " = " + coq(x.third.convert(), sb3, fk1, fk2, "") + ".\n");			
		}
		for (var en : dst.ens) {
			sb2.append("Parameter " + coq(en.toString()) + " : Set.\n");	
		}

		
		for (var en : dst.ens) {
			for (var fk : dst.fksFrom(en)) {
				sb2.append("Parameter " + coq(en.toString()) + "_" + coq(fk.toString()) + " : " + coq(en.toString()) + " -> " + coq(dst.fks.get(fk).second.toString()) + ".\n");					
			}
			for (var att : dst.attsFrom(en)) {
				sb2.append("Parameter " + coq(en.toString()) + "_" + coq(att.toString()) + " : " + coq(en.toString()) + " -> " + coq(dst.atts.get(att).second.toString()) + ".\n");									
			}
		}
		for (var x : dst.eqs) {
			sb2.append("Axiom ax" + (i++) + " : " + toCoq(x.first.first, x.first.second) + " " + coq(x.second, sb3, fk1, fk2, "") + " = " + coq(x.third, sb3, fk1, fk2, "") + ".\n");			
		}

		for (var en : src.ens) {
			for (var fk : src.fksFrom(en)) {
				Term tt = Term.Var("x");
				var p = Term.Fks(fks.get(fk).second, tt); //.stream().map((k)->coq(k.toString())).collect(Collectors.toList());
				var q = coq(p, sb3, fk1, fk2, "");
				sb2.append("Definition t_" + coq(en.toString()) + "_" + coq(fk.toString()) 
				+ " : " + coq(ens.get(en).toString()) + " -> " + coq(ens.get(src.fks.get(fk).second).toString())
				+ " := fun x => " + q + ".\n");						
			}

			for (var att : src.attsFrom(en)) {
				var p = atts.get(att).third; //, tt); //.stream().map((k)->coq(k.toString())).collect(Collectors.toList());
				var q = coq(p, sb3, fk1, fk2, "");
			
				sb2.append("Definition t_" + coq(en.toString()) + "_" + coq(att.toString()) 
				+ " : " + coq(ens.get(en).toString()) + " -> " + coq(src.atts.get(att).second.toString())
				+ " := fun " + coq(atts.get(att).first) + " => " + q + ".\n");									
			}
		}
		Function<Att1, En1> fk1x = (k)->src.atts.get(k).first;
		Function<Fk1, En1> fk2x = (k)->src.fks.get(k).first;

		for (var x : src.eqs) {
			sb2.append("Conjecture conj" + (i++) 
					+ " : " + toCoq(x.first.first, ens.get(x.first.second))
					+ " " + coq(x.second, sb3, fk1x, fk2x, "t_") + " = " +
					coq(x.third, sb3, fk1x, fk2x, "t_") + ".\n");			
		}
	...
	public synchronized Schema<Ty, Chc<En1, En2>, Sym, Chc<Chc<Fk1, Fk2>, En1>, Chc<Att1, Att2>> collage() {
		...
		DP<Ty, Chc<En1, En2>, Sym, Chc<Chc<Fk1, Fk2>, En1>, Chc<Att1, Att2>, Void, Void> dp = new DP<>() {
			...
			@Override
			public boolean eq(Map<String, Chc<Ty, Chc<En1, En2>>> ctx,
		...
		Set<Triple<Pair<String, Chc<En1, En2>>, Term<Ty, Chc<En1, En2>, Sym, Chc<Chc<Fk1, Fk2>, En1>, Chc<Att1, Att2>, Void, Void>, Term<Ty, Chc<En1, En2>, Sym, Chc<Chc<Fk1, Fk2>, En1>, Chc<Att1, Att2>, Void, Void>>> eqs2 = new THashSet<>();
		for (Triple<Pair<String, En1>, Term<Ty, En1, Sym, Fk1, Att1, Void, Void>, Term<Ty, En1, Sym, Fk1, Att1, Void, Void>> eq : src.eqs) {
	...
	public static <Ty, En1, Sym, Fk1, Att1, En2, Fk2, Att2, En3, Fk3, Att3> Mapping<Ty, En1, Sym, Fk1, Att1, En3, Fk3, Att3> compose(
		...
		Map<Att1, Triple<String, En3, Term<Ty, En3, Sym, Fk3, Att3, Void, Void>>> atts0 = new THashMap<>(
				m1.atts.size());
		for (Att1 att1 : m1.atts.keySet()) {
			En3 en3 = m2.ens.get(m1.atts.get(att1).second);
			String v = m1.atts.get(att1).first;
	...
	private static <Ty, En2, Sym, Fk2, Att2, En3, Fk3, Att3> Term<Ty, En3, Sym, Fk3, Att3, Void, Void> subst(
		...
		if (t.var != null) {
			...
			Triple<String, En3, Term<Ty, En3, Sym, Fk3, Att3, Void, Void>> x = m2.atts.get(t.att());
	...
	public Morphism<Ty, En1, Sym, Fk1, Att1, Void, Void, En2, Sym, Fk2, Att2, Void, Void> semantics() {
		...
		semantics = new Morphism<>() {
			...
			@Override
			public Pair<Map<String, Chc<Ty, En2>>, Term<Ty, En2, Sym, Fk2, Att2, Void, Void>> translate(
					Map<String, Chc<Ty, En1>> ctx, Term<Ty, En1, Sym, Fk1, Att1, Void, Void> term) {
				Map<String, Chc<Ty, En2>> map = new THashMap<>(ctx.size());
	...
	public Map<String, Chc<Ty, En2>> trans(Map<String, Chc<Ty, En1>> ctx) {
		Map<String, Chc<Ty, En2>> ret = new THashMap<>(ctx.size());
		for (String v : ctx.keySet()) {
	...
	public final Map<Att1, Triple<String, En2, Term<Ty, En2, Sym, Fk2, Att2, Void, Void>>> atts;
	...
	public static <Ty, En, Sym, Fk, Att> Mapping<Ty, En, Sym, Fk, Att, En, Fk, Att> id(Schema<Ty, En, Sym, Fk, Att> s,
		...
		Map<Att, Triple<String, En, Term<Ty, En, Sym, Fk, Att, Void, Void>>> atts = Util.mk();
		String v = ("v");
		...
		return new Mapping<>(ens, atts, fks, s, t.get(), true);
	}

	public Mapping(Map<En1, En2> ens, Map<Att1, Triple<String, En2, Term<Ty, En2, Sym, Fk2, Att2, Void, Void>>> atts,
	...
	public void validate(boolean doNotCheckEquations) {
		...
		for (Att1 att1 : src.atts.keySet()) {
			Triple<String, En2, Term<Ty, En2, Sym, Fk2, Att2, Void, Void>> att2 = atts.get(att1);
			if (att2 == null) {
		...
		if (!doNotCheckEquations) {
			for (Triple<Pair<String, En1>, Term<Ty, En1, Sym, Fk1, Att1, Void, Void>, Term<Ty, En1, Sym, Fk1, Att1, Void, Void>> eq : src.eqs) {
				...
				boolean ok = dst.dp.eq(Collections.singletonMap(eq.first.first, Chc.inRight(ens.get(eq.first.second))),
						lhs, rhs);
				if (!ok) {
					throw new RuntimeException("Source schema equation " + eq.second + " = " + eq.third
							+ " translates to " + lhs + " = " + rhs
							+ ", which is not provable in the target schema.  To proceed, consider removing it from the source or adding more equations to the target.");
				}
			}
		}
	...
	public synchronized String toString() {
		...
		for (En1 en : src.ens) {
			...
			for (Fk1 fk : src.fksFrom(en)) {
				fks0.add(fk + " -> " + (fks.get(fk).second.isEmpty() ? "identity " + fks.get(fk).first
						: Util.sep(fks.get(fk).second, ".")));
			}
