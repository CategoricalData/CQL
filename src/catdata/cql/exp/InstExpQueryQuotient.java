...
import catdata.cql.fdm.SigmaChaseAlgebra;
...
public class InstExpQueryQuotient<Gen, Sk, X, Y> extends InstExp<Gen, Sk, Integer, Chc<Sk, Pair<Integer, Att>>>
  ...
  public SchExp type(AqlTyping G) {
    for (Block b : queries) {
      ...
      for (Pair<String, String> s : b.gens) {
        if (!s.second.equals(b.en)) {
          throw new RuntimeException("From clause should match (the written down) entity of " + b.en);
        }
      }
    }
    return I.type(G);
  ...
  public synchronized Instance<String, String, Sym, Fk, Att, Gen, Sk, Integer, Chc<Sk, Pair<Integer, Att>>> eval0(AqlEnv env,
      boolean isC) {
    Schema<String, String, Sym, Fk, Att> sss = I.type(env.typing).eval(env, isC);
    Set<String> ensX = new THashSet<>();
    for (Block b : queries) {
      ensX.add(b.en);
    }
    ...
    Query<String, String, Sym, Fk, Att, String, Void, Void> q = Query.makeQuery(ens, Collections.emptyMap(),
        Collections.emptyMap(), Collections.emptyMap(), sss, dst, op);
    

    Instance<String, String, Sym, Fk, Att, Gen, Sk, X, Y> I0 = I.eval(env, isC);
    if (queries.isEmpty()) {
      return (Instance<String, String, Sym, Fk, Att, Gen, Sk, Integer, Chc<Sk, Pair<Integer, Att>>>) I0;
    }

    //System.out.println("q " + q);
    //System.out.println("I0 " + I0);
    EvalInstance<String, String, Sym, Fk, Att, Gen, Sk, String, Void, Void, X, Y> J = new EvalInstance<>(q, I0, op);
  //  System.out.println("J " + J);
    
    boolean useChase = (boolean) op.getOrDefault(AqlOption.quotient_use_chase);

    if (useChase) {
      Map<String, Set<Pair<X, X>>> mm = new THashMap<>(ens.size());
      J.gens().keySet((p)->{
        Map<String, Chc<X, Term<String, String, Sym, Fk, Att, Gen, Sk>>> m = p.asMap();

        List<String> vs = new TreeList<>(m.keySet());
        String v1 = vs.get(0);
        String v2 = vs.get(1);
        Chc<X, Term<String, String, Sym, Fk, Att, Gen, Sk>> x1 = p.get(v1);
        Chc<X, Term<String, String, Sym, Fk, Att, Gen, Sk>> x2 = p.get(v2);
        String en = J.gens().get(p);
  ...
  //  return  Util.anomaly();
      SigmaChaseAlgebra<String, String, Sym, Fk, Att, String, Fk, Att, Gen, Sk, X, Y> alg = new SigmaChaseAlgebra<>(
  ...
  private Instance<String, String, Sym, Fk, Att, Gen, Sk, Integer, Chc<Sk, Pair<Integer, Att>>> evalProver(AqlEnv env,
      Instance<String, String, Sym, Fk, Att, Gen, Sk, X, Y> I0,
      EvalInstance<String, String, Sym, Fk, Att, Gen, Sk, String, Void, Void, X, Y> J) {
    Collage<String, String, Sym, Fk, Att, Gen, Sk> col = new CCollage<>(I0.collage());

    List<Pair<Term<String, String, Sym, Fk, Att, Gen, Sk>, Term<String, String, Sym, Fk, Att, Gen, Sk>>> eqs0 = new LinkedList<>(
        );
    ...
    J.gens().keySet((p) -> {
      ...
      Chc<X, Term<String, String, Sym, Fk, Att, Gen, Sk>> x1 = p.get(v1);
      Chc<X, Term<String, String, Sym, Fk, Att, Gen, Sk>> x2 = p.get(v2);
      Term<Void, String, Void, Fk, Void, Gen, Void> t1 = I0.algebra().repr(p.en2(), x1.l);
      Term<Void, String, Void, Fk, Void, Gen, Void> t2 = I0.algebra().repr(p.en2(), x2.l);
      if (!t1.equals(t2)) {
    ...
    InitialAlgebra<String, String, Sym, Fk, Att, Gen, Sk> initial0 
    = new InitialAlgebra<>(strat, I0.schema(), col, (y) -> y, (x, y) -> y);

      
    LiteralInstance<String, String, Sym, Fk, Att, Gen, Sk, Integer, Chc<Sk, Pair<Integer, Att>>> ret = new LiteralInstance<>(
