...
public final class TyExpRaw extends TyExp implements Raw {
  ...
  private final Collage<String, Void, Sym, Void, Void, Void, Void> col;
  ...
  public TyExpRaw(List<TyExp> imports, List<LocStr> types, List<Pair<LocStr, Pair<List<String>, String>>> functions,
    ...
    col.tys().addAll(this.types.stream().map(x -> (x)).collect(Collectors.toList()));
    for (Pair<String, Pair<List<String>, String>> x : this.functions) {
      col.syms().put(Sym.Sym(x.first, x.second), x.second);
    }
    ...
    for (Entry<String, Triple<List<String>, String, String>> kv : Util.toMapSafely(this.java_fns).entrySet()) {
      List<String> l1 = kv.getValue().first.stream().map(x -> (x)).collect(Collectors.toList());
      var zzz = new Pair<>(l1, (kv.getValue().second));
      Sym s = Sym.Sym(kv.getKey(), zzz);
      col.syms().put(s, zzz);
  ...
  @SuppressWarnings("unchecked")
  static Triple<Map<String, Chc<String, Void>>, Term<String, Void, Sym, Void, Void, Void, Void>, Term<String, Void, Sym, Void, Void, Void, Void>> infer1x(
      Map<String, Chc<String, String>> Map0, RawTerm e0, RawTerm f, Chc<String, Void> expected,
      @SuppressWarnings("rawtypes") Collage col, String pre, AqlJs<String, Sym> js) {
    return RawTerm.infer1x(Map0, e0, f, (Chc<String, String>) ((Object) expected), col, pre, js).first3();
  }

  private static Map<String, String> conv3(Map<String, String> m) {
    return Util.map(m, (x, y) -> new Pair<>((x), y));
  }

 // private static Pair<List<String>, String> conv2(List<String> k, String v) {
 //   List<String> l1 = k.stream().map(x -> (x)).collect(Collectors.toList());
 //   return new Pair<>(l1, (v));
 // }

 // private static Map<Sym, Pair<List<String>, String>> conv1(Map<String, Pair<List<String>, String>> m) {
  //  return Util.map(m, (k, v) -> new Pair<>(Sym.Sym(k, v), conv2(v.first, v.second)));
 // }

  public void doGuiIndex(List<LocStr> types, List<Pair<LocStr, Pair<List<String>, String>>> functions,
    ...
    for (Pair<LocStr, Pair<List<String>, String>> p : functions) {
      f.add(new InteriorLabel<>("functions", new Triple<>(p.first.str, p.second.first, p.second.second), p.first.loc,
          x -> x.first + " : " + Util.sep(x.second, ",") + (x.second.isEmpty() ? "" : " -> ") + x.third).conv());
    }
    raw.put("functions", f);
  ...
  public synchronized String toString() {
    ...
    if (!java_fns.isEmpty()) {
      ...
      for (Pair<String, Triple<List<String>, String, String>> sym : Util.alphabetical(java_fns)) {
        temp.add(
            sym.first + " : " + fff.apply(sym.second.first) + sym.second.second + " = " + Util.quote(sym.second.third));
      }
  ...
  public synchronized TypeSide<String, Sym> eval0(AqlEnv env, boolean isC) {
    ...
    AqlJs<String, Sym> js = new AqlJs<>(ops, col.syms(), col.java_tys(), col.java_parsers(), col.java_fns());

    for (Triple<List<Pair<String, String>>, RawTerm, RawTerm> eq : eqs) {
      try {
        Triple<Map<String, Chc<String, Void>>, Term<String, Void, Sym, Void, Void, Void, Void>, Term<String, Void, Sym, Void, Void, Void, Void>> tr = infer1x(
    ...
    Set<Triple<Map<String, String>, Term<String, Void, Sym, Void, Void, Void, Void>, Term<String, Void, Sym, Void, Void, Void, Void>>> eqs0 = col
        .eqsAsTriples().stream().map(x -> new Triple<>(xxx(x.first), x.second, x.third)).collect(Collectors.toSet());

    TypeSide<String, Sym> ret = new TypeSide<>(col.tys(), col.syms(), (eqs0), col.java_tys(), col.java_parsers(),
        col.java_fns(), ops);

    return ret;
  }

  private static Map<String, String> xxx(Map<String, Chc<String, Void>> x) {
    return Util.map(x, (k, v) -> new Pair<>(k, v.l));
  }

  static Map<String, Chc<String, String>> yyy(List<Pair<String, String>> l) {
    Map<String, Chc<String, String>> ret = (new THashMap<>(l.size()));
    for (Pair<String, String> p : l) {
      if (ret.containsKey(p.first)) {
        throw new RuntimeException("Duplicate bound variable: " + p.first);
      }
      Chc<String, String> x = p.second == null ? null : Chc.inLeft((p.second));
      ret.put(p.first, x);
    }
    return ret;
  }
