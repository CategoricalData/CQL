...
import catdata.cql.SkeletonExtensionalInstance;
import catdata.cql.SkeletonInstanceWrapperInv;
...
public final class InstExpRandom extends InstExp<Integer, Integer, Integer, Integer> implements Raw {
  ...
  @Override
  public synchronized Instance<String, String, Sym, Fk, Att, Integer, Integer, Integer, Integer> eval0(AqlEnv env,
      boolean isC) {
    if (isC) {
    ...
    Schema<String, String, Sym, Fk, Att> schema = sch.eval(env, false);

    for (String e : schema.ens) {
      if (!ens.containsKey(e)) {
        ens.put(e, 1);
      }
    }
    for (String e : schema.typeSide.tys) {
      if (!ens.containsKey(e)) {
        ens.put(e, 1);
      }
    }

    Map<String, Integer> en = new THashMap<>();
    for (String x : schema.ens) {
      int y = this.ens.get(x);
      en.put(x, y);
    }
    Map<String, Integer> ty = new THashMap<>();
    for (String x : schema.typeSide.tys) {
      int y = this.ens.get(x);
      ty.put(x, y);
    }
    ...
    Map<String, Integer> n = new THashMap<>();
    for (String x : schema.ens) {
      m.put(x, t);
      t += this.ens.get(x);
    }
    ...
    Map<Att, Term<String, Void, Sym, Void, Void, Void, Integer>[]> atts = new THashMap<>();

    for (String e : schema.ens) {
      int e_size = this.ens.get(e);
      for (Fk f : schema.fksFrom(e)) {
        int[] xx = new int[e_size];
        fks.put(f, xx);
        String y = schema.fks.get(f).second;
        int size0 = this.ens.get(y);
      ...
      for (Att f : schema.attsFrom(e)) {
        ...
        Term<String, Void, Sym, Void, Void, Void, Integer>[] xx = new Term[e_size];
        atts.put(f, xx);
        String y = schema.atts.get(f).second;
        int size0 = this.ens.get(y);
        for (int a = 0; a < e_size; a++) {
    ...
    @SuppressWarnings("unchecked")
    Term<String, Void, Sym, Void, Void, Void, Integer>[][] talg_eqs = new Term[0][];
    BiPredicate<Term<String, Void, Sym, Void, Void, Void, Integer>, Term<String, Void, Sym, Void, Void, Void, Integer>> talg_dp = (
        x, y) -> Util.anomaly();

    SkeletonExtensionalInstance<String, String, Sym, Fk, Att> J = new SkeletonExtensionalInstance<>(schema, en, ty, m, n,
