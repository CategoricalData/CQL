...
public final class MapExpRaw extends MapExp implements Raw {
	...
	private Map<String, Integer> enPos = new THashMap<>();
	...
	public Mapping<String, String, Sym, Fk, Att, String, Fk, Att> eval0(AqlEnv env, boolean isC) {
		...
		Collage<String, String, Sym, Fk, Att, Void, Void> dcol = new CCollage<>(dst0.collage());

		Map<String, String> ens0 = new LinkedHashMap<>(ens.size());
		// Map<String, Pair<String, List<String>>> fks0 = new HashMap<>();
		Map<Att, Triple<String, String, Term<String, String, Sym, Fk, Att, Void, Void>>> atts0 = new LinkedHashMap<>(
				atts.size());
		Map<Fk, Pair<String, List<Fk>>> fksX = new LinkedHashMap<>(fks.size());

		for (MapExp k : imports) {
			Mapping<String, String, Sym, Fk, Att, String, Fk, Att> v = k.eval(env, isC); // env.defs.maps.get(k);
		...
		Util.putAllSafely(ens0, Util
				.toMapSafely(ens.stream().map(x -> new Pair<>((x.first), (x.second))).collect(Collectors.toList())));
		...
		for (Pair<Pair<String, String>, List<String>> p : this.fks) {
			...
			try {
				String start_en_fixed = ens0.get(x);

				String start_en = ens0.get(x);
		...
		for (Pair<Pair<String, String>, Triple<String, String, RawTerm>> att : atts) {
			try {
				...
				Pair<String, String> p = src0.atts.get(Att.Att((att.first.first), att.first.second));
				if (p == null) {
				...
				Map<String, Chc<String, String>> ctx = Collections.singletonMap(var, var_en2);

				Term<String, String, Sym, Fk, Att, String, String> term0 = RawTerm.infer1x(ctx, term, null,
						proposed_ty2, dcol.convert(), "", src0.typeSide.js).second;

				Util.putSafely(atts0, Att.Att((att.first.first), att.first.second),
						new Triple<>((var), dst_att_dom_en, term0.convert()));
			} catch (RuntimeException ex) {
				ex.printStackTrace();
				throw new LocException(attPos.get(Att.Att((att.first.first), att.first.second)),
		...
		Mapping<String, String, Sym, Fk, Att, String, Fk, Att> ret = new Mapping<>(ens0, atts0, fksX, src0, dst0,
