...
public class InstExpSpanify<X, Y> extends InstExp<X, Y, X, Y> {
	...
	public synchronized Schema<String, String, Sym, Fk, Att> makeSchema(AqlEnv env, AqlOptions ops) {
...
//    }
		...
		Map<String, List<X>> gens = Util.mk();
		String en = ("R");
		...
		var ret = new Instance<String, String, Sym, Fk, Att, X, Y, X, Y>() {
			...
			public DP<String, String, Sym, Fk, Att, X, Y> dp() {
				return new DP<String, String, Sym, Fk, Att, X, Y>() {
					...
					private Term<String, String, Sym, Fk, Att, String, String> transL(
							Term<String, String, Sym, Fk, Att, X, Y> term) {
						if (term.obj() != null) {
							return term.convert();
						} else if (term.sym() != null) {
							List<Term<String, String, Sym, Fk, Att, String, String>> l = new ArrayList<>(
									term.args.size());
							...
							return Term.Att(Att.Att("R", term.att().str), transL(term.arg));
					...
					@Override
					public synchronized boolean eq(Map<String, Chc<String, String>> ctx,
							Term<String, String, Sym, Fk, Att, X, Y> lhs,
							Term<String, String, Sym, Fk, Att, X, Y> rhs) {
						System.out.println("? " + lhs + " = " + rhs + " after  + " + transL(lhs) + " = " + transL(rhs));
