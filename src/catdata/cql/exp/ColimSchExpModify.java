...
public final class ColimSchExpModify extends ColimSchExp implements Raw {
	...
	public synchronized ColimitSchema<String> eval0(AqlEnv env, boolean isC) {
		...
		for (Pair<String, String> k : ens) {
			colim0 = colim0.renameEntity(k.first, k.second, checkJava).first;
		}
		...
		for (Pair<Pair<String, String>, String> k : atts0) {
			colim0 = colim0.renameAtt(Att.Att((k.first.first), k.first.second), Att.Att((k.first.first), k.second),
					checkJava).first;
		}
		for (Pair<Pair<String, String>, List<String>> k : fks) {
			...
			Term<String, String, Sym, Fk, Att, String, String> t = RawTerm.infer1x(Map, term, null, Chc.inRight(tr),
					xxx.convert(), pre, colim0.schemaStr.typeSide.js).second;

			colim0 = colim0.removeFk(Fk.Fk((k.first.first), k.first.second), t.toFkList(), checkJava).first;
		}
		for (Pair<Pair<String, String>, Triple<String, String, RawTerm>> k : atts) {
			...
			Term<String, String, Sym, Fk, Att, String, String> t = RawTerm.infer1x(Map, k.second.third, null,
					Chc.inLeft(r.second), xxx.convert(), pre, colim0.schemaStr.typeSide.js).second;
			colim0 = colim0.removeAtt(Att.Att((k.first.first), k.first.second), (k.second.first), t.convert(),
					checkJava).first;
		}
