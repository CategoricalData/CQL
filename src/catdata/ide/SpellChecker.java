...
class SpellChecker extends AbstractParser {
    ...
    for (Token t : doc) {
      if (t.isComment()) {
        ...
        while (matcher.find()) {
          ...
          if (word2.length() > 0 && !getWords().contains(word2)
              && !local.apply(Unit.unit).contains(word.replace("`", "").replaceAll("\"", "")
  ...
  private void spellingError(String word, int off) {
    ...
    SpellingParserNotice notice = new SpellingParserNotice(this, text, line, off, word.length());
  ...
  private static class SpellingParserNotice extends DefaultParserNotice {
    ...
    private SpellingParserNotice(SpellChecker parser, String msg, int line, int offs, int len) {
