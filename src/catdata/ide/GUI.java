...
import catdata.cql.gui.WarehouseUI;
...
public class GUI extends JPanel {
	...
	public static Pair<AqlEnv, String> getCurrent() {
		...
		CqlCodeEditor e = (CqlCodeEditor) c;
...
//	    KeyStroke commandW = KeyStroke.getKeyStroke(KeyEvent.VK_W, InputEvent.META_DOWN_MASK);
	...
	private static Menu makesToolsMenu() {
		...
		// MenuItem validator = new MenuItem("SQL Validator");
		// toolsMenu.add(validator);
		// validator.addActionListener(x -> SqlValidatorUI.show());

		MenuItem wh = new MenuItem("Warehousing");
		toolsMenu.add(wh);
		wh.addActionListener(x -> WarehouseUI.show());

//		MenuItem wh2 = new MenuItem("ConexusDB");
//		toolsMenu.add(wh2);
//		wh2.addActionListener(x -> ConexusDB.show());

//		MenuItem wh3 = new MenuItem("Excelerator");
//		toolsMenu.add(wh3);
//		wh3.addActionListener(x -> new Excelerator().showIt());

		/*
		 * MenuItem wh4 = new MenuItem("Wellview Upgrade Calculator");
		 * toolsMenu.add(wh4); wh4.addActionListener(x -> { JFileChooser chooser = new
		 ...
		 * 
		 * String f = chooser.getSelectedFile().getAbsolutePath(); if (f == null)
		 * return; String xxx = ""; try { xxx = WellviewUpgrader.go(f); } catch
		 * (Exception ex) { ex.printStackTrace(); xxx = "# " + ex.getMessage(); }
		 * newAction(null, xxx, Language.getDefault()); });
		 */
	...
	private static Menu makeEditMenu() {
		...
		/*
		 * MenuItem clear = new MenuItem("Refresh Outline"); editMenu.add(clear);
		 * clear.addActionListener(x -> { CodeEditor<?, ?, ?> ed = getSelectedEditor();
		 * if (ed != null) { ed.doUpdate(); } });
		 */
	...
	private static JPanel makeToolBar() {
		...
		/*
		 * JButton abortB = new JButton("Stop"); abortB.addActionListener(e -> {
		 * CodeEditor<?, ?, ?> ed = (CodeEditor<?, ?, ?>)
		 * editors.getComponentAt(editors.getSelectedIndex()); if (ed != null) {
		 * ed.abortAction(); } });
		 */

		/*
		 * JButton deployB = new JButton("Deploy"); deployB.addActionListener(e -> {
		 * CodeEditor ed = (CodeEditor)
		 * editors.getComponentAt(editors.getSelectedIndex()); if (ed != null) {
		 * ed.deployAction(); } });
		 */
	...
	private static Menu populateAql() {
		...
		st.addActionListener(x -> AqlTester.doSelfTestGui());
		menu.add(st);
		...
		ih.addActionListener(x -> {
			...
			if (c instanceof CqlCodeEditor) {
				CqlCodeEditor a = (CqlCodeEditor) c;
				a.emitDoc();
			}
		});
		menu.add(ih);
		...
		/*
		 * MenuItem html = new MenuItem("Visual Edit"); html.addActionListener(x -> {
		 * CodeEditor<?, ?, ?> c = getSelectedEditor(); if (c == null) { return; } if (c
		 * instanceof AqlCodeEditor) { AqlCodeEditor a = (AqlCodeEditor) c;
		 * a.visualEdit(); } });
		 */
		// menu.add(html);
...
//		deployItem.addActionListener(e -> deployAction());
	...
	private static void doExample(Example e) {
		...
		newAction(e.toString().trim(), e.getText().replace("\r", ""), e.lang()); // TODO rsyntaxtextarea parsing
	}
	...
	/*
	 * private static void abortAction() { CodeEditor<?, ?, ?> c =
	 * getSelectedEditor(); if (c != null) { c.abortAction(); } }
	 */
...
//	    s = s.replace("\r", ""); // TODO CQL may not be enough the \r make get added by output stream
	...
	private static void formatActionAql() {
		...
		CqlCodeEditor cc = (CqlCodeEditor) c;
		cc.format();
	}
