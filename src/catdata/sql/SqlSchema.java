...
public class SqlSchema {
	...
	public SqlSchema(DatabaseMetaData meta, String tick) throws SQLException {
		...
		try (ResultSet result0 = meta.getTables(null, null, "%", new String[] { "TABLE" })) {
			while (result0.next()) {
				...
				String preX = result0.getString(2);
				String x = result0.getString(3);
				table.name = new Pair<>(preX, x);
				//System.out.println("whoo " + table.name);
				
				if (table.name.first != null && (table.name.first.toLowerCase().equals("system") || table.name.first.toLowerCase().equals("sys") || table.name.first.toLowerCase().equals("ctxsys") || table.name.first.toLowerCase().equals("xdb")) ) {
				...
				try (ResultSet cols = meta.getColumns(null, table.db, result0.getString(3), null)) {
					while (cols.next()) {
					//	System.out.println("whee "  +cols.getString(4));
				...
				try (ResultSet pks = meta.getPrimaryKeys(null, table.db, result0.getString(3))) {
					while (pks.next()) {
			...
			/*
			 * try (ResultSet result = meta.getTables(null, null, "%", new String[] {
			 * "TABLE" })) { while (result.next()) { String preX = result.getString(2);
			 * String xx = result.getString(3); SqlTable table = getTable(new Pair<>(preX,
			 * xx));
			 * 
			 * // name, local cols, foreign cols List<Triple<List<String>, List<Pair<String,
			 * String>>, String>> fks0 = new LinkedList<>(); try (ResultSet rfks =
			 * meta.getImportedKeys(null, table.name.first, table.name.second)) { int
			 * lastSeen = 0; List<String> l = new LinkedList<>(); List<Pair<String, String>>
			 * r = new LinkedList<>(); String foreignTable = null; String foreignColumn =
			 * null; // String localTable = fks.getString(7); String localColumn = null;
			 * String fkname; String lfk = null; while (rfks.next()) {
			 * 
			 * fkname = rfks.getString(12); // 13 is pkey name, // should // be irrelevent
			 * foreignTable = preX + rfks.getString(3); foreignColumn = rfks.getString(4);
			 * // String localTable = fks.getString(7); localColumn = rfks.getString(8);
			 * String seq = rfks.getString(9); if (Integer.parseInt(seq) <= lastSeen) {
			 * fks0.add(new Triple<>(l, r, lfk)); l = new LinkedList<>(); r = new
			 * LinkedList<>(); } lastSeen = Integer.parseInt(seq);
			 * 
			 * l.add(localColumn); r.add(new Pair<>(foreignTable, foreignColumn)); lfk =
			 * fkname; } if (foreignTable != null || foreignColumn != null || localColumn !=
			 * null) { fks0.add(new Triple<>(l, r, lfk)); } }
			 * 
			 * for (Triple<List<String>, List<Pair<String, String>>, String> xxx : fks0) {
			 * SqlForeignKey fk = new SqlForeignKey(); fk.source = getTable(table.name);
			 * fk.target = getTable(xxx.second.get(0)); fk.name = table.name + "__" +
			 * Util.sep(xxx.first, "_") + "__" + Util.sep( xxx.second.stream().map(x ->
			 * x.first + "_" + x.second).collect(Collectors.toList()), "_"); // xxx.third ==
			 * null ? "FK" + (fkidx++) : xxx.third; int i = 0; for (String lc : xxx.first) {
			 * Pair<String, String> fc = xxx.second.get(i);
			 * fk.map.put(fk.target.getColumn(fc.second), fk.source.getColumn(lc)); i++; }
			 * fks.add(fk); } } }
			 */
		}
		validate();
	...
	private final Map<Pair<String, String>, SqlTable> tableMap = new THashMap<>();

	private SqlTable getTable0(Pair<String, String> name) {
	...
	public SqlTable getTable(Pair<String, String> name) {
	...
	public boolean isTable(Pair<String, String> name) {
		SqlTable ret = getTable0(name);
		return (ret != null);
	}
	...
	private final Map<Pair<String, String>, Set<SqlForeignKey>> fksFrom0 = new THashMap<>();

	private Set<SqlForeignKey> fksFrom(Pair<String, String> name) {
	...
	private final Map<Pair<String, String>, Set<SqlForeignKey>> fksTo0 = new THashMap<>();

	public Set<SqlForeignKey> fksTo(Pair<String, String> name) {
